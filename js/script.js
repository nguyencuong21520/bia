...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const toggleRsvpModal = () => {
    const modalContent = rsvpModal.querySelector(
      'div[class*="bg-\\[\\#fffdf5\\]"]'
    );
    const mobileDrawer = document.getElementById("mobile-drawer");
    const drawerBackdrop = document.getElementById("drawer-backdrop");
    ...
    if (rsvpModal.classList.contains("hidden")) {
      ...
      rsvpModal.classList.add("items-center", "justify-center");
      ...
      setTimeout(() => {
        ...
        rsvpModal.classList.remove("items-center", "justify-center");
  ...
  const musicIndicatorMobile = document.getElementById(
  ...
  const toggleMusic = () => {
    if (isPlaying) {
      ...
      if (musicIconMobile)
        musicIconMobile.classList.remove("animate-spin-slow");
      ...
      bgMusic
        ...
        .then(() => {
          ...
          if (musicIconMobile)
            musicIconMobile.classList.add("animate-spin-slow");
          if (musicIndicator) musicIndicator.classList.remove("hidden");
          if (musicIndicatorMobile)
            musicIndicatorMobile.classList.remove("hidden");
  ...
  if (addToCalendarBtn) {
    addToCalendarBtn.addEventListener("click", () => {
      const event = {
        ...
        details:
          "Mời cô chú đến dự tiệc thôi nôi của Bé Bia! Cùng chung vui với gia đình nhé.",
      ...
      const url = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(
        ...
        event.details
      )}&location=${encodeURIComponent(event.location)}&ctz=Asia/Ho_Chi_Minh`;
  ...
  if (viewMapBtn && mapSection) {
    viewMapBtn.addEventListener("click", (e) => {
      ...
      const mapSectionTop = mapSection.offsetTop - navHeight - 20; // 20px padding
